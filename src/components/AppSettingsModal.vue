<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm transition-all duration-300">
    <div class="bg-[#141414] rounded-xl shadow-2xl w-[1000px] h-[700px] max-w-full max-h-[90vh] flex overflow-hidden border border-gray-800 ring-1 ring-white/10">
      ...
      <div class="flex-1 flex flex-col min-w-0 bg-[#0f0f0f]">
        ...
        <div class="flex items-center justify-between px-6 py-3 border-b border-gray-800 bg-[#141414]">
          <div class="flex flex-col">
            <h3 class="text-white font-medium tracking-wide text-sm uppercase">{{ currentMenuItem?.label || 'Settings' }}</h3>
             <span class="text-gray-500 text-xs mt-0.5" v-if="currentMenuItem?.tip">
              {{ currentMenuItem.tip }}
            </span>
          </div>
          <button @click="close" class="text-gray-400 hover:text-white transition-colors p-1.5 hover:bg-gray-800 rounded-full">
