  ...
  >
    ...
    >
      ...
      <div class="flex-1 flex flex-col min-w-0 bg-[#0f0f0f]">
        <!-- Header -->
        <div
          class="flex items-center justify-between px-6 py-3 border-b border-gray-800 bg-[#141414]"
        >
          <div class="flex flex-col">
            <h3 class="text-white font-medium tracking-wide text-sm uppercase">
              {{ currentMenuItem?.label || "Settings" }}
            </h3>
            <span
              class="text-gray-500 text-xs mt-0.5"
              v-if="currentMenuItem?.tip"
            >
              {{ currentMenuItem.tip }}
            </span>
          </div>
          ...
          <Transition name="fade" mode="out-in">
            <div :key="activePanel" class="h-full">
              ...
              <AppSettingsService
                ...
                :loading-text="serviceLoadingText"
                :logs="serviceLogs"
                @start="startService"
                @stop="stopService"
              ...
              <!-- Config Editor Panel -->
              <AppSettingsConfig
                v-else-if="currentMenuItem?.type === 'config'"
                :content="configContent"
                :type="currentMenuItem.configType || 'text'"
                :filename="currentMenuItem.file"
                :loading="configLoading"
                :has-backup="hasBackup"
                :message="configMessage"
                :message-type="
                  configMessageClass.includes('red') ? 'error' : 'success'
                "
                @save="saveConfig"
                @restore="restoreConfig"
                @change="(val) => (configContent = val)"
              />
              ...
              <AppSettingsMeilisearch
                v-else-if="activePanel === 'meilisearch-keys'"
                :app="app"
              />
            </div>
          </Transition>
...
const baseMenuItems = [
  {
    id: "service",
    label: "Service",
    tip: "Start, stop, or restart the service",
  },
];
...
const currentMenuItem = computed(() =>
  menuItems.value.find((m) => m.id === activePanel.value),
);
...
const checkServiceStatus = async () => {
  ...
  serviceLogUnsubscribe = window.sysapi.apps.onServiceLog(
    props.app.id,
    (data) => {
      if (data.type === "status") {
...
const startService = async () => {
  ...
  const args =
    props.app.service_commands?.start || props.app.default_args || "";

  try {
    await window.sysapi.apps.startService(
      props.app.id,
      props.app.execPath,
      args,
    );
...
const stopService = async () => {
  ...
  try {
    await window.sysapi.apps.stopService(
      props.app.id,
      props.app.execPath,
      args,
    );
...
const restartService = async () => {
  ...
  try {
    await window.sysapi.apps.restartService(
      props.app.id,
      props.app.execPath,
      startArgs,
      stopArgs,
...
const loadConfig = async () => {
  ...
  const appDir =
    lastSlash > 0
      ? props.app.execPath.substring(0, lastSlash)
      : props.app.execPath;
...
const saveConfig = async (content) => {
  ...
  try {
    const result = await window.sysapi.apps.saveConfig(
      appDir,
      configFile,
...
const restoreConfig = async () => {
  if (
    !confirm(
      "Are you sure you want to restore the configuration from backup? Current changes will be lost.",
    )
  )
    return;
