  ...
  <div class="h-full flex flex-col bg-[#1e1e1e] rounded-lg border border-gray-800 overflow-hidden shadow-sm">
    ...
    <div class="flex items-center justify-between p-2 border-b border-gray-800 bg-[#1a1a1a]">
      ...
      <div class="flex items-center space-x-2">
        <span v-if="message" class="text-xs mr-3 slide-in" :class="messageType === 'error' ? 'text-red-400' : 'text-green-400'">
            {{ message }}
        </span>
        ...
        <button
          ...
          class="flex items-center px-3 py-1.5 bg-gray-800 hover:bg-orange-900/40 text-gray-300 hover:text-orange-400 disabled:bg-gray-800 disabled:text-gray-600 disabled:cursor-not-allowed rounded border border-gray-700 hover:border-orange-500/50 transition-all text-xs"
        ...
        <button
          @click="save"
          :disabled="loading"
          class="flex items-center px-4 py-1.5 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:cursor-not-allowed rounded text-white text-xs font-medium shadow-md shadow-blue-900/20 transition-all"
        >
           <svg v-if="loading" class="animate-spin -ml-1 mr-2 h-3.5 w-3.5 text-white" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
           <svg v-else class="w-3.5 h-3.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"></path></svg>
          Save
        </button>
      </div>
    </div>
