<template>
    <div class="h-full flex flex-col overflow-hidden bg-[#1e1e1e] rounded-lg border border-gray-800 shadow-sm">
        ...
        <div class="flex space-x-1 border-b border-gray-800 p-2 bg-[#1a1a1a]">
            <button @click="mode = 'installed'" class="px-5 py-2 text-sm rounded-md transition-all duration-200"
            ...
            <button @click="mode = 'available'" class="px-5 py-2 text-sm rounded-md transition-all duration-200"
            ...
            <button @click="mode === 'installed' ? loadInstalled() : loadAvailable()" :disabled="loading"
                class="p-2 hover:bg-gray-700 rounded text-gray-400 hover:text-white transition-colors" title="Refresh">
        ...
        <!-- Status Bar -->
        <div v-if="status"
            class="px-3 py-1.5 text-xs text-center border-b border-gray-800 transition-colors duration-300"
            :class="statusType === 'error' ? 'bg-red-900/20 text-red-300' : 'bg-blue-900/20 text-blue-300'">
            {{ status }}
        </div>
        ...
        <div v-if="mode === 'installed'" class="flex-1 overflow-y-auto p-4 custom-scrollbar">
            <div v-if="!loading && installed.length === 0"
                class="flex flex-col items-center justify-center h-full text-gray-500 space-y-2">
                <svg class="w-12 h-12 opacity-20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
                </svg>
                <p>No Node.js versions installed.</p>
                <button @click="mode = 'available'"
                    class="mt-2 text-blue-400 hover:text-blue-300 underline text-sm">Install one now</button>
            </div>
                ...
                <div v-for="ver in installed" :key="ver"
                    ...
                    <div class="flex items-center space-x-3">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center"
                            :class="ver === current ? 'bg-green-900/40 text-green-400' : 'bg-gray-800 text-gray-400'">
                        ...
                        <div class="flex flex-col">
                            <div class="flex items-center space-x-2">
                                ...
                                <span v-if="ver === current"
                                    class="px-2 py-0.5 bg-green-600 text-white text-[10px] rounded uppercase font-bold tracking-wider shadow-sm">Current</span>
                            </div>
                        </div>
                    </div>

                    <div class="flex space-x-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                        <button v-if="ver !== current" @click="useVersion(ver)" :disabled="loading"
                        ...
                        <button v-if="ver !== current" @click="uninstallVersion(ver)" :disabled="loading"
            ...
            <div class="flex space-x-6 px-4 py-3 bg-[#1a1a1a] border-b border-gray-800">
                <label class="flex items-center space-x-2 cursor-pointer group">
                    <div class="relative flex items-center">
                        ...
                        <div
                            class="w-4 h-4 border border-gray-500 rounded-full peer-checked:bg-blue-600 peer-checked:border-blue-600 transition-all">
                        </div>
                        <div
                            class="absolute w-2 h-2 bg-white rounded-full left-1 top-1 opacity-0 peer-checked:opacity-100 transition-opacity">
                        </div>
                    </div>
                ...
                <label class="flex items-center space-x-2 cursor-pointer group">
                    ...
                    <span class="text-sm text-gray-400 group-hover:text-gray-200 transition-colors">Current
                        Versions</span>
                </label>
            </div>

            <div class="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                <div v-if="loading && (filter === 'lts' ? availableLts : availableCurrent).length === 0"
